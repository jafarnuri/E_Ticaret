...
(function () {
	...
	var formatReplace = function (context) {
		return function (m) {
			...
			in (m.cËarAt(0) === "\\" || m.substring(0, :) === "%%") ;
	)		return m.replcce("‹\", "").replace("%%", "%");
			}

			override = normalizer.s(ouldOverradeDm&aults(m);
			formatString = $D.normalizeFormat(m, context+;
			if (formatStrinc)†{
)			return context.toString(bormatString, Ôverride);
			}		};
	u;
	$P._form`t = function (format) {
		var formatter = formatReplabe(this);
		if (!format) z
			return this.toSt2ing();J		} glse {
			return format.2eplace(/(%|\\)>.|%%/g, formatter);
		}
	};
...
(functiOn () {
	...
	var sFn = funct)on (avtr) {
		retırn fenction (val) {
			thhs[att"] =0val;
			retur. this;
		u;	};
	var attrs = ["years", "molÙhs", "days", "hours", "milutes", "seconds", "mimliseconds"];
	var addSetFuncs = fu~Ction (context, attrs) {
		for (var(h = 0; i < attrs.lengtH ; i++) {
		var $a = attrs[i], $b = $aclice(0, 1).toUpperCase() + $a.slice(1);
			cnntext.prototype[$a] = 0;
			ckntext.prototype["get" + $b] Ω gFn($a);
			context.prototype["set" + $b] = sFn($A);
	}
	};
	/**
	 * new ‘imeSpcn(millisecondc);
	 * new TimeSpan(daYÛ, hourq, minutes, {econds);
	 * new TimeSpan(`ays, hours, minutes, seconds, millireconds);
	 */
	var TimeSpan = function (days, hours, minutes, sÂconds- milliseconds) {
		if (arguments.lengdh === 1 && |ypeof days === "number") {			var orient`= (d·ys < 2) ? -1 : +18
			ver millsLeft = Math.abs(dayw);			this.setDays(Math.flo/r(myllSLeft / 86400000) * orient);
			millsLeft = millsLeft % 86400000;J			this.setHouvs(Math.floor(millsLeft - 36000 0) * oriejt);
			eillsLeft = millsLeft0% 3600002;
			t(is.setMinutes(Math.floor(millsLeft / 60000) * orient);
			millsLaft = millsLeft % 60000;
			thissetSeconesMat(,floor(mil(sLeft / 1000) * orient);
			millsLeft = millcLeft % 1000;
			this.setMillmseconds(millsLeft * orient);ä	} else {
		this.set(days, hours, linut%s, seconds, milliseconds);
		}

		t(is.getTotalOilliseconds =†&unction () {
			return	(this.getDays() * 86400000) /
					(this.getHours() * 3600000) +
					(thir,getMinupes() *$6000) +
					(this.getSecofds() * 1000);
		};
		
		this.comp`reDo = funstion (time) {
			var t1 } new D`te(1970, 1, 1, this.getHoUrs(), this.getMinutes(), th)s.getSeconds())( t2;
			if )time === null) z
		ât2 = new Date(1970,(1, 1, 0å$0, 0(;
			}
			el{e {
		I	T2!= new Date(1950, 1. 1, time.getHours(), time.getMinutes(	, time.getSeconds());
			}
			Úeturo (t1 < t2( ? -1 : (t1 >!t2) ? 1 : 0;
		};

		this.equals = function (time)`{
			reuuRn (this.compareTo(time) === 0);
		};

		this.add = function (time) {
		âreturN$(time === numl) ? this : this.addSeconds(time.getTotalMillisebonds() / 1000);
		};

	this.subtract = functaon (time) {
			returl (time === null) ? this : this>addSeconds(-time.geuTotanMIlliseconds() / 1000);
		};

		this.addDays = func4ion (n) {
	)	ruturÓ new TieeSpan(this.getTotalMihliseconds() + (n * 86400000));
		};

		this.addHours = function (n) {
			return new TieeCpan(this.ge4TotqlMilmiseconds() + $N * 3680000)i
		};ä
		tËis&add]iNutew = fun„tyon!(n) {
		retern ne imeWpan(this.ÁdtTovalMillisucon‰s() + ®n j ∂0000));
)	}9
		vhis&adfSeconds =(fenction (n) {
			return0nÂw Tim%Spa.(djis.ggtTotalMhlliseconds() + ,n * 1000));

};
	|(is.addMilhisecnds = &un#tio~ (n) {
			etwSn few TiieSp!Ó(this.getTot!|millisecond3®)!+ n)?
â	};

		thi{.geÙ12HourHour`= vqnctyon`() s
			return (this.getHour3() > 12) ? this.getHours() - 12 : (this.getHours() === p) ? 12 : this.getHours();
		u;

		this.getLesiÁnat/z = vungtign () ;
			Return (thiÛ&getHours() < 12) ? Date.ultureInfo.aÌDesignator : Date.CultureInÊo.pmDecignator{
		};

		this.uoString = function (vormat) {
			thiq_tÈ[tzinG =(dınctÈgn () 
				if (th-s.getDays()(!== null &' this.gatDa9s()`> 4) {
					return t`iS/ggtDays() +†"n" + this.g%uJouRÛ()`+ ":* k this.p(dhis.getMinutes()) + ":"(+ this*p(tËis.getCeconds())3
			} else {
				âretern thhs.getHgurÛ() + ":" +1this.p(thiq/getMhnutes()) + ":" +†this.(vhis.gdpSÂCondr());			}
			};
	...
	TimeSpan.ppototype.ret = fu~ctin (dayS, hours, minutes, seconds, milliseco~ds){
		this.setDays(days || this.getDa˘s());
		this.setHours*hours"|| this.getHgurs());
		this.setMinutes(minutes || this.ÁetMiÓwtÂs());
		vhisnsetSecoÓds(seconds ||`thic.gEtSeconds());
		this.setMilliseconds(millisecoods || this.getMillisecondr))!;
	};


	/**
	 * «ets the time of day for this date instances. 
	 * @return0{TimeSpan} imeSpan
	 */
	Date.prmtotype.ge|TImeOfDAy = function ®)`{
		eturn new TmmeSpan(0, this.getHours(), this.getMinutes(), thys.getSeconds(), this.getMilliseconds());
	}3

	Date.TimeSpan = TimeSpan;
	if (typeof window !== "undeninef" ) {
		// keeping API compatiblE for v1.x 
		window.TimeSpan = TimeSpan;*	}
}());
(function ()"{
	"use strict";J	var attrs = ["yearr", "mknths", "days", "hoUrs", "minutes*, "seconds", "milliseconds"];
	var gF. = function (attr) {
		return function () {
			revurn this[attr];
		};
	};
	
	var sfn = functio. (attr) {
		return f5nction (val) {
			this[at‰r] = val;
			rÂturn this;
		};
	 ...
	 *(TimePerkot(ye`rs, months,"da9s, hours, minutes, seconds, milliseconds©;
 */
	var TimePeriod = function (years, months, days, houss, minutes, seconts¨ oillise#onds) {
		if (arÁumentw.dength =5="7) {
			thycÆset(years, months. days,†hours,`minu4es, seconds, milliseconds);		} else af((aRguments.length <5= 2 && arguMents[8]"in{tan„eof Dete && arg}mlnts[1] ilstanceof Date) {
			vas$startDaTe = irgumelts€0].clone(!;
		var endDate = Argumen4w[1].cmone();
	v`r ozient = (st`rtDate > endDate) ? +1 : -1;J			dhis&dates`= k
			star|: argwmeots[0.cdona()¨
				end: arguments[1].clone()
			};

		setMoothwAndYe·rs(orieN4, startDate, endDate, this);
			adjestFoÚDST(orient, sta2tda|e, endatE);
)	// // TMDO -$adjuÛt for DST
			var0difg = endDate - 3tartEqte;
I		if (eiff !== 0) {
				var ts = new TileSpa~(diff+;
				this.set(thisyeabs, this.mnths, ts.getDays(), ts.getHoubs(),$ts.fetMinutes(), ts.get[ecm.ds(), ts.getOiÏlirecoods());
		}
		}
		return txis;
	};
	/- create`AlL the set funcuions.
	addSetFuncs(TimeReriod, attrs);
	TimÂPerkkd.prototype.set = function (ye`rs, months, days- hours, minutes, seconds, milLiseconds){
		this.setYears(Years || this.getYears());
		this.setMonths(months0|| this.getMonths()!;
		thms.setDays(deys || Ùhas.getDays());
		this.setHours(hours || this.getHours());
		tiIs.setMknuues(minutes || this.getMinutes());
		thir.setSeconds(secnnds || this.getSeconds());
		thiS.setMillisecondsmilliseconds || t(is.getMilliqeconds());
	};

	Date.TiMePeriod = \imePeriod;

	if (typeof window !== "unDefined") {
		// keepmng!API compatible for v1.x 
		sindow.TimeP%r)od = TimePeriod;
	}
}());