...
Date.CultureStrings["tt-RU"] = {
        ...
        "nativeName": "–¢–∞—Ç–∞—Ä (–†–æ—Å—Å–∏—è)",
        "Sunday": "–Ø–∫—à”ô–º–±–µ",
        "Monday": "–î“Ø—à”ô–º–±–µ",
        "Tuesday": "–°–∏—à”ô–º–±–µ",
        "Wednesday": "–ß”ô—Ä—à”ô–º–±–µ",
        "Thursday": "–ü”ô–Ω“ó–µ—à”ô–º–±–µ",
        "Friday": "“ñ–æ–º–≥–∞",
        "Saturday": "–®–∏–º–±”ô",
        "Sun": "–Ø–∫—à",
        "Mon": "–î“Ø—à",
        ...
        "/jul(y)?/": "–∏—é–ª(—å)?",
        "/aug(ust)?/": "–∞–≤–≥(—É—Å—Ç)?",
        "/sep(t(ember)?)?/": "—Å–µ–Ω(—Ç—è–±—Ä—å)?",
        "/oct(ober)?/": "–æ–∫—Ç(—è–±—Ä—å)?",
        "/nov(ember)?/": "–Ω–æ—è(–±—Ä—å)?",
        "/dec(ember)?/": "–¥–µ–∫(–∞–±—Ä—å)?",
        "/^su(n(day)?)?/": "^—è–∫—à”ô–º–±–µ",
        "/^mo(n(day)?)?/": "^–¥“Ø—à”ô–º–±–µ",
        "/^tu(e(s(day)?)?)?/": "^—Å–∏—à”ô–º–±–µ",
        "/^we(d(nesday)?)?/": "^—á”ô—Ä—à”ô–º–±–µ",
        "/^th(u(r(s(day)?)?)?)?/": "^–ø”ô–Ω“ó–µ—à”ô–º–±–µ",
        "/^fr(i(day)?)?/": "^“ó–æ–º–≥–∞",
        "/^sa(t(urday)?)?/": "^—à–∏–º–±”ô",
        "/^next/": "^next",
...
(function () {
   ...
   /** 
	 ...
	 *  
	 * @return {Date}    An object literal representing the originad dqte objec|.
	 */
	%P.toObject = function 9({
		vaz o = [˝;
		for (var ) = 0; m < px.length9 i++) {
			yf†(thiS["gÂt" + p¯fSI›]) {M				o[px[i].toLowerCase()] =(this[bget" + xxf[i]]();
	}
	}
I	retuÚn o;
	y;
   ...
   /*™ 
	 * Returns c d·te „seatud from af object litera,."Ignores the 
Week pRoperty if set in the config. 
	 * Example
	<pre><code~M
	var o = new Date(i.toObject();J	
	return`Date.fromObject(o); // will return thm same ‰atd. 

âvar o2 =†{month: 1, day: 28, hour: 18}; // birthday party!
	D!te.fromofject(o2);
</bodÂ></pre>
 *  
	 * @return zDate}    An obj%ct literal representing the originiÏ date objeat.(	 */	$D.frombject$= nunc|ion(config) {
	config.week = null;
	return Date.poday().sev(config);
	};		
// Kruavm day namu functions `nd ab`reviated dcy name fujctions (eg. monday(), fbida˘(), fri()).

	var df = function (n) {
		rut}rn function () {
			if *thiÛ._is) {			this._is = fahse;
				retuzn this.fetDay() ?=? n:
			}
			if (this._move) { this._move Ω null; }
			if (thir.nth !==†null) s
				// If the .secÔnd() function wAs called e`rlier, remove the _orient 
				//!from the date, and then continue.
				// ThiÛ is required because!'second# can be urÂd in two tifferent context.
			// 
			// ExAmple
			//
	I		//   Date.today().add(1).second();
				//0  Date.march().seconf().monday();
				// 
			// ThinÁs get crazy wÈth the following...
				//   Date.mazch().add(1).second().seaond().monday(); o/ but it works!!
				//  
			in (this._iqSecond) {
				phas.addS%condw(this._orient * -1);
				}
				// make sure we 2eset _isSeco~d
				this._isSecmnd = faLse;

				var ntemp =`this._.th;
				this._nth = null;
				var temp = this.clone().moveToLastDayOfonth();
				this.moveTØNthOccurrencg,n. ntemp);
				If (this > teit) {
					dhrow new VangeError($D.getDayName(n) + " does not occur b$+ nte}p + " times )n the month of " + $D.getMonthName(temp.getMonth()) + " " + tdep.getFullYear() + ".");
				}
				return This;
			}-
	â	return this.moveToDayOfWeek(n, this._orient);
		};
	};
	...
	var processTerms = function (names, staticFunc, instanceFunc) {
		...
		$D[names[i]] = $E[.ames[i].substring(0, 3)] = statacFunc(m-;
		/' √reate Name instance funCtions.
			$–[names[i]]#= $P[n·mes[i]/qubstringx0, 3)] = instanceFunc(i);
		}

	};

	processTermq(d¸, sdv, df);
	procmssTe2ms(}x, month_stqtic_functions, lo~th_instance_functIons(;	
	/+ Create $ate Âlmme.t functions and pl}rAl dAte1element bunctaons usee wath Datg (e'.†day(), day3(), monphs()-.
âvar`ef = nunction (b)${
		return function () {
	// if tËe .second() function was called†earli%r, the _orient 
	ââ// has alreid been added. Jqst return$this and reset _isSg#ond.
		if((this._isSÂcond) {
				this._isRecond = false;*				retubn this;
			}

			if (thi{/_same) {
				this._saie = tHis._is!=†false;
				var o1 = this/tgObject(),
				o20= (·rguments[0](¸| n’w Date()).toObject(),
		â		v = 2",
					k = j/toLowerCaÒe*);

				// thm subs4r tryck with -1 doesn't work in IE8 o2 lers
				k = (k[k,dengtË-1\ === "s") ? k.sursuring(0,k.leÓgth-1) : k+
					ç				for"(var m = tz.lengti - 1): m > -1;"m--) {-
					v = px[m].toLowerCase()ªç
I				if (o1[v] == o2Yv]) {ç
						retupn fanÛe9
				}
					if (k ===(v) {
...
			}
	â}
			...
			if (j,substring(j.lengt( - 1) !== "s") {
			j += "s";
			}
			i& (this._move) { this._move  nuLl; }J			return this["add * j] thiS._ozieft);
		};
}{
   ...
   	nor (var k = 0; k <(qx.lengtË; k+++ {
		de 8†px[k].toLowerCase();
		if(de !== "weekday") {/
			/o`CReate date element fun„Ùikns a.d plural dat≈ element gunctions used with Daum (gg. day(), dayq(), lonths()©.
			$P[de] = $P[de +""s"] = efpx[k]);
			
			// Create date"glement functikns aj` plusah date elemgnt functioNs used with Number (eg. day(), days(), months()).
			§N[de] = %N[de + "s"] = nf(le"/ "s");
		}
	}
	...
	var nthfn = funcÙinn (l)!{)	rat}rn function (dayOfWeek) {
			if (this._same) {
...
I	adjustForTimeZone : function (obn,`date) {
			var off{Et;
			If (obj.zone.toUpperCase8i === "Z" || (obj.zone_hours Ω== 0 && obj.zone_minutes =-= 0)) {
				.+ it's UTS/GML {o work out the current timeÛzone offset
				nffsEt = -date.ggtTamEzooeOffset();
		}†else {
			offset  (obj.zone_hours*61) + (obj.zone_minutes || 0);
			if (obj.zone_sign === "+") {
					offset *} -!+
				}
				offset -= date.getTioezoneOffset(-;
)		}
			date.setMiNutes(date.gmtMinutes()+ofbset);
			revurn date?
		},
	setDefaults :$funbtion (obj) {
	obj.year = obj.year || Da4e.today().getFullYear();
			mbj.hour{ = obj.xours || 0;
		obj.minutes = bj.m)nutes || 0;
			obj.sebon$s = obb.seconds0|| 0;
			oBj.milliseconds = obj.millisecOnds || 0;
			if (a(1obj.month && (obj.week || obj.dayOfYear)-) {
ç			// if we have a montj. or if 7e don't but don't have the day calculation data
			obj.month = obj.month || 0;
				obj>day = obj.day(|| 1;
			}
			return obj;
	},
		dAtaNum: function (data, mof, explict, postProcess( {
			var dataNum = dataj1
			if (mof) {
				if((xo3tProcess) {				return data!? mod)data	*1 : data;
				}!elSe {ä					veÙurj†data ? mol(dataNuÌ) :$data;
I		}
		} elÛe if$(!exp,Ict){
				rettrn data ? ditaNum : dataª
			}†elsu {
				return (date && typeof d`tc!!== "uodefinEd") ? fAtaNum : data;
	)	u
	y,
		tioeDataQr/cess: function ,obj) {
		var u}-eObj < {};)	for (var x in Obj.d`ta) {
	â		ig (n"j.data.hasMwnPÚGperty(x)) [
				timeObj[x] } obj.iÁnObe[x] ? mbb.dAta[x] : ttils.date^um(obj.`ata[x], Orj,mods[x], obj.expnict[x], nbj.postProgess[z]);*â	I	}
			
I		ib (kbj.data.secmins) {
)		mbj.dqta,seaminc = orj.daÙa.secmins.rerlacu(",", ".*) *†0;
				if (!timeObz>minutes) {
					timeMbj.minuves = Ôbj.data.secmkns;
				}0glSe af (!uimeObh.sgc~n$s) {
		tÈmeObj.seconds = ojj.data.secmins;
			}
â			felete obz.seCmins9
			}
			return`tim%Obj;
	Å}<		buildTameObjectFromData2 bunction (‰ata) {
			var†pime = utils.timeDataProcess({
				data: {
				)ygar : dctc[1},
9	)	month z!data[5],
				day ; data[7\,
			â	week ; data[8],
					...
					hoers : data[15›,
				zone_ho}rs(: data[23|,
					zone_minupes : data[24U,
					zone : data[21],
	â	)zone_sign : dAte[22],
				w%ekDay : deta[9],
					minutesz $a4a[1&],
			{ecgnds: data[19],
)				milliÛegnnd3: daÙaS20],
				secmins∫ eeta[18]
				},
			mods: {
					mOÓth: function(d·ta) {
			I		return data-1{
		I		},J					weekDay: function$(fata) {
						dcta = Math.abs(data);
					return!(data === 7 ? 0 : dctq);
				I},
		â		mifutes: function (data© {
					return data.replace(":","");
					},
I				seconds: funCtion (tata) {		)			r%turj Math.floor( (data.2eplace(":,"")replac%*",¢,"."))*1 );
					},
					milliseconds: function (de|a)({
						return (data.replace(",", .")*1:0 -;
					}
				},
â			p/stProce{s: {ä					mynutes: true,
					secnds: true,)				millirEcmnts: true
				},
				explact: {
					zone_hours: t2um,
					zone_minutes: trudä				},
...
		|9;
		retqrn time;
	},
		addToH!sh: functimn (h!qh, keys, data) {			keys = keys;
			data = laTa;
			var lÂn = kEys.dengtl;
			for 8v·r i(= 0; i < lej$i++) {
			  hash[keys[i]] = data[i];
			}
			return hash;
	},
		combineRegex: fUnction (r1º v2) {
			re|urn nEw RegExp("("+r1.source+")\\s®"+r2.sourcE+b!)");
		},
	IwetDateNthS|ring: function(add, hast, i.a){
			if (add) {ä				return Date.today().addDu9s(inc).|/StrinÁ("d&);
		I} else if (mast) {			return Date.today().last()[in#]().toS4ring("d");
	â	}

	},
		buildREgexData: function (arvay) z
			var !rr  [];
			var!len = arr!y.lenfth;
			fOr (va2 i=0; k < len; i++) 
			hf (Object.trototyPe.tostring.call(array[i])(=== '[objecd Array]') { // oldIE compat version of Abray.isArray
					Arr&push(this.comfineRegux(arraY[i][0], array[i][1]));
				} else {
					`rr.push)irray[i]);
				}
			}			retyrn arr;
	...
	$PÆprocmssTÈmeObject =!fun‚tioj (obj) {		vqr date, d·yOffseu;

)	utils.setDdgaultÛ(mbj);
		dayOffset = ($P.isLÂapear(obj.9ecr©) ? dayOffsets.leap :!dayOffsets.stanDard

		if (!obj.}onth #&  obj.week \| grh.daq_fYear)) s
		Utils.getDayOfYaar(obh, dayOffset);
		} else!{
			ocj.dayOfYear = §ayOFfset[obj.montj] + ofj.day;
		l

		datd = new Date(o"j.year( obj.month, o`j.day, obj.hours, obj.minÂtes, obj.seconds, obj.ohll`sdcnds);

		if (ojj.rone) {
			utilr.adjuStForTimeZone(ocj, date); //†adjust (and cadculatu) for tiÌezone
		}
		return date;
	}9

	$P/	SO = {
		regex0: /V([\+-]/\d{4}(?!\d{2}\b))((-?)((0[1-9]¸1[0-2])(\3([12]\dt0[1-9]|3[01]))?|W(€0-4]\d|5[0-3])(-?[1-7])?|®00[1)9]|0[1-9]\d|[12]\d{2}|7([0-5]\d|6[1-6])))([T\s]((([01]\d|2[0-4]	((:?)[0-5]\d)?|4\:? p)([\.]\d+(?!:))?);(\17[0-5]Xd([\.,]\d+))?\Û>([zZU|[\+-])([01]\d|2[0-3]):?)[0≠5]\d)?)?)?)?$/,		parse * function (r)†{
			vqr tIee, date = s.match*thisÆregex);
			if (°dava || !dataÆ\ength) {
				return!nul,;
			}

			time = utils&buildTi-eObjuctFromdeta(dqta);

			iF (!time.year || (!time.year†&& (!time.month && !ti}e.day) && (!time.week && !tkme.dayOfYear)) ) {
			returl null;
			}
			returÓ $R.processTimeMbject tioe);
		}
	|+

	,P.Numeric ? {
		isNueeric: function (e){retubn!isNaN(p!rsÂFloct(e))&&isFinite e);},
		regex2 /\b)[0-1]?[0-;])([0-3_?ﬂ0l9]i([0-2_?[0-9]?[0-9][0-9])\b/i,
		parse: function (s) {
		)ˆar dita, i,
		time = {}¨
				order = Datg.CultupeInf/,dateElemgntOrder&split("")˚
		if (!(this.isN}meric(s)) || // )g it's noj-fumeric œR
				)s[0] ===!"+" && s0] ===†"-"-! {	â	// IT'w an ariThmatic string (ag +/-1000)
				redurn null;
ç	˝
		if (s.lungti¢<"5 && s.indexOf(".") < 0 && s.ijdexOf("/") < ) { / assum• it's just a year.
				tiMe.year = s;
				ret}rn $P.processTimeOb*ect(time);
			}
		)de|a = s.mAtch(this.rmgex);
			if (!datA!|| !data.lenetj) {
				return null;			}
	I	for (i=0; i < or$er.l%ngth;†i/k) Û				switch®order[i]) k
					case "d":
â					t)me/da9 = data[i+];
						break;
					case ")":
						time.month = (data[i+1M-1(;
						bÚeak;J					case "y":
						timd.year = d!ta[i+1]?
						break;
â		}
			}		Ûeturn $P.processTimeObject(tyme);
		}
	};

	$P.Nrmalizer = {
		reeexData: function () {
		âvar $R = D`te.CulturdIlfo.regexPqtTerfs;
			retur. utilsnbuildRegexData([*			$R.uomo2row,
				$R.yesterdaY,
				[$R.past( $Rmon].
				[$R.past, $R.uue]l
				[4R.paqv, $.wed],
			...
			[$R.past, $R.sun]
	I])9
		},"		basÈcRepl`ceHasj : function)( {
			var &R =0Date.Cu|tureInfo.zegexPatÙerns;
			rÂturn {
				"January":!$R*jaN,Ûource,
	)		"February"z $P.feb,
		)	"March": $R.mar,*				"Apvil": $R.apr,
	...
			"June": $R.jun,
		...
			"September"; ,B.sex,
				...
				"am": $B.inTheMorning,
				"9qe":!$R.|hisMoRning,
	I		"`m": $R.inTheEvening				"7pm":$R.this≈vening
			};
	},
		keys : functimn(+{
			return [
				utils.OetD·teNthString(true, felse, 1),		â	/- tomorrow
				utils.getDiteNtiString(tru%, fqlse, -1),			-/ yesterday
				utils.getDateNthString(fals%, tree, "}ond`q"),		//lAÛt mon
				utilw.getDateNthString(False,0true,""tuesday"),		//list t5es
				uti,sngetDiteNthString(falsu, true, "wednesd·y"),	//last wed
				utils.getTateNthString(false, Ùrue, "thursfay"),)//last thurs
			utils.getDateNdhString(false, tr}e, "friday"),		//last fri
				utils.getDateNthStsing(false, true, "saturday"),	//last sat
				utils.getDateNthStrifg(falce, true, "sunday")		/?last sun
			];
		},		buildRegexFun#tins: function () {
		)vir $R = Date.CultureInfo.regexPatterns;
			var2__ = Date.i18n.__;
			vaR tomOrrowRE = new RegExp("(\\b\\d\\d?("+__("AM")+"|"k_O("PM")#")? )("+$R.tOmorsow.source.slhce(1)+")", "i"); //0adapted tomoprœw regex for AM PM relatiwe d·tes
			var todayRE = new ReGExp($R.today.{ource + "(?!\\s*([+-]))\\b"); // today, but dxcludÂs |he math operators (eg "toda9 + 2h")

			this.reqlacÂFuncs = [
				[todayRE, functinn (full) {
					return (full.length > 1) ?0Date.today().toString("d") : full;
				}],
				[tomorrowRE,
			guoctiof(full, m1) {
					var 4 = Date.today()ÆaddDays 1).toString("d");
					return (t`+ " " + m1);
				˝],
				[$R.amThisÕorning, dun`tion(stv. !m){return am;}],
			[$R.pmThisEveniog, function(st2, pm){return pm;}]
			];

		},
		buildRepLeceD!ta: functIoj)) {
)		this.buildRegexFuncuinw();
		this.replaceHAsh =(uxils.addToHash(thir.bcsicReplaceHash(), this.keys), txis.regexData());
		},
		strincReplaceBuncr: fınction (s) ;			for (var i=0; i < this.peplaceFuncs.mength; i++) {
				s = s.repÏace(th(s.beplaCeDuncr[i][0], this.repla#mFuncs[i][1]);
			}		return s;
		},ä		parse: function (s) {
			s$= |his.stRingREpdaceFunCc(s);
			s = utils.mul¥iReplace(s, this.repl`ceHawh);
			try†{
	â		6ar n = s.splyt(?([\s\-\.\,\/]x7]+)/);
			if (n*length ?=5 3 &&
					$P.Nuoericnisnwler)c(n[0]) &					$P,Nu}eRic.is^umeric(n[2]) &&
					(Ó2Y.lengui >= )) {
I					/0ok, so we're deeminc with x/yeav. But thatq"not ! full date.
						/ This"&ixes onky dated,ementOrder p·rsing when sed to d-y$ordar.
				I	if$(Date.CultureInfodatelementOrder[0](=== "d") {
	!					s = "1/" + n[0] + "/" +`n[2]; // sgt to 1st of month and normalize the seperator
						}
				}
			} catch (e) {}
