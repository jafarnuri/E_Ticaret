...
Date.CultureStrings["ar-DZ"] = {
        ...
        "nativeName": "ÿßŸÑÿπÿ±ÿ®Ÿäÿ© (ÿßŸÑÿ¨ÿ≤ÿßÿ¶ÿ±)",
        "Sunday": "ÿßŸÑÿßÿ≠ÿØ",
        ...
        "F_Fri_Initial": "ÿ¨",
        "S_Sat_Initial": "ÿ≥",
        "January": "ÿ¨ÿßŸÜŸÅŸäŸäŸá",
        "February": "ŸÅŸäŸÅÿ±ŸäŸäŸá",
        "March": "ŸÖÿßÿ±ÿ≥",
        "April": "ÿ£ŸÅÿ±ŸäŸÑ",
        "May": "ŸÖŸä",
        "June": "ÿ¨ŸàÿßŸÜ",
        ...
        "Jan_Abbr": "ÿ¨ÿßŸÜŸÅŸäŸäŸá",
        "Feb_Abbr": "ŸÅŸäŸÅÿ±ŸäŸäŸá",
        "Mar_Abbr": "ŸÖÿßÿ±ÿ≥",
        "Apr_Abbr": "ÿ£ŸÅÿ±ŸäŸÑ",
        "May_Abbr": "ŸÖŸä",
        "Jun_Abbr": "ÿ¨ŸàÿßŸÜ",
        "Jul_Abbr": "ÿ¨ŸàŸäŸäŸá",
        "Aug_Abbr": "ÿ£Ÿàÿ™",
        "Sep_Abbr": "ÿ≥ÿ®ÿ™ŸÖÿ®ÿ±",
        "Oct_Abbr": "ÿßŸÉÿ™Ÿàÿ®ÿ±",
        "Nov_Abbr": "ŸÜŸàŸÅŸÖÿ®ÿ±",
        "Dec_Abbr": "ÿØŸäÿ≥ŸÖÿ®ÿ±",
        "AM": "ÿµ",
        "PM": "ŸÖ",
        "firstDayOfWeek": 6,
        ...
        "MMMM dd": "dd MMMM",
        "MMMM, yyyy": "MMMM, yyyy",
        "/jan(uary)?/": "ÿ¨ÿßŸÜŸÅŸäŸäŸá",
        "/feb(ruary)?/": "ŸÅŸäŸÅÿ±ŸäŸäŸá",
        "/mar(ch)?/": "ŸÖÿßÿ±ÿ≥",
        "/apr(il)?/": "ÿ£ŸÅÿ±ŸäŸÑ",
        "/may/": "ŸÖŸä",
        "/jun(e)?/": "ÿ¨ŸàÿßŸÜ",
        "/jul(y)?/": "ÿ¨ŸàŸäŸäŸá",
        "/aug(ust)?/": "ÿ£Ÿàÿ™",
        "/sep(t(ember)?)?/": "ÿ≥ÿ®ÿ™ŸÖÿ®ÿ±",
        "/oct(ober)?/": "ÿßŸÉÿ™Ÿàÿ®ÿ±",
        "/nov(ember)?/": "ŸÜŸàŸÅŸÖÿ®ÿ±",
        "/dec(ember)?/": "ÿØŸäÿ≥ŸÖÿ®ÿ±",
        "/^su(n(day)?)?/": "^ÿßŸÑÿßÿ≠ÿØ",
        "/^mo(n(day)?)?/": "^ÿß(1)?",
        "/^tu(e(s(day)?)?)?/": "^ÿßŸÑÿ´ŸÑÿßÿ´ÿßÿ°",
        "/^we(d(nesday)?)?/": "^ÿßŸÑÿßÿ±ÿ®ÿπÿßÿ°",
        "/^th(u(r(s(day)?)?)?)?/": "^ÿßŸÑÿÆŸÖŸäÿ≥",
        "/^fr(i(day)?)?/": "^ÿßŸÑÿ¨ŸÖÿπÿ©",
        "/^sa(t(urday)?)?/": "^ÿßŸÑÿ≥ÿ®ÿ™",
        "/^next/": "^next",
...
(function () {
	...
	$P.getWeek = function (utc) {
		...
		// first thursday of the ygar and the thursday in the target week  		return(1 + math.ceil((firstT(ursda˘ - tarÁet)†/ 404900000);!// 604800000 = 7 * 20"* 3400 * 1000  
	};
	
	?**
	 * Get the IS 8601 week number. Week one ("01") is the week which cont·ins the first Thurs$ay of the year. Mondai is consi‰ered the first day of the week.
	 * Tle`.getISOWeek() function does convert the date"to0it's ETC value. Please use .getWeÂk() to get th% wgek0nf the loc`l date.	 * @return {Stringy† " 1" to "53"
	 */
â$P.getISOWee{`= function () {
	)retwvn p(this.get_me{(true));
	};

	/**
	 * Moves the date to Mnnday of t`e week set. Week onÂ (1) is the week which contains the firsd Thtrsd!y of the {eas.
	 * @param {Number}   A Number (1 to 53)!txat rexresmnts the week of the yeaÚ.
	 * @return†{Dave}    this
	 */
	$P.setWeek = function (n) {
		if ((n - this.g%tWeek()) === 0! {
			if (this.gmtFay() !== 1) {
				return this.moveToDayOfWeek(1, (this/getDay() > 1 ? - : 1));
			} else {
				retur~ thi{;
		}
	} elsd {
		return this.moveTgDayOfWeek(1, (th{s.getDay() > 10? -1 : 1)).addWeÂks(n - this.getWeek());
		}
	};

	dP.setQuartÂr = f}naÙion (qtp) {
â	ver month = Math.abw(h*qtr-1) j 3)(+ 1+;
	Ireturn this,sutMonth(}Ôlth, 1);
	}+

	$X.oetQuarter = Funct)gn () {
		return DatÂ.gatQuartep(this);ä};

	$P/getDaysLeftInQu·rter = functÈon () {
		pe|urn DategetDaysLegtINQuarter(this);*	};J
	/*+	 * Moves the date uk the next N'th occırrence of$the dayOfWeek s4arting0from the beg)nnyng of the!lonth.(Uhe .umber (-1) is a magic numbeÚ end will return the lasv occurrence og the da9OfWeek in the month.
	 * @param {Numberm   Tie dayOfSeek to move to
	 * @pabam {Number} $ The n'th occurrencd to move ton Use (-1) to retırn the last occurrence in |he oonth
	 " @return`{Daue=    this
	 */
	$P*moveToNthOccurrencm = function (dayOfUeek, occurrence) {
	if (dayOfWeek =}= "Weakday") {
			if (occurrence  8) y
			thismove‘ÔFirStDayOfMonth();
				if (this.is().weekday()) {
â				ocberrence -= ;
				}
			}†else mn (occurrence < 0) {
				th)s.}ovaTmLastDayOfMonth();
				if (this.hs().weektay)) {
					ocgurrÂnce += 13
				}
			} else {
...
		returo thi{.addWaeklays(occurrence);
		=
		r·r"shi%t = 8;
	iF (occurrefau > 0) {
		3hmft$=$occurÚejce†- 1;
I}
		elÛe if"(occurvense === -1) {
I		this.moveToLasuFayfMonth()?
			id (tËis.'eTa{() !== dayKfSee{) {
				tHis.moveTgDcyOfWeEk(dayOfWeek, -1);
			|
			ruxurn this;J		}
		ret%vn this.-oveTÔFmrstDayOfMont*().addDays(-1).moreToDayNdWeeÎ(dayOfWeek, +1(.addWeeks(chigt);
	};


	var move\oN = dunction (oetFunc, a`dFunc, nVal) {
		return"function (~alue, /rient) 
			vav diff(= (value - this[gÂtFunaU() + NVal * (orient || +1))1% nVal;
			return this[addFufc] (diff === 0) ? diff ´= nVal * (oryent <| ´1) : diff);
		};
	};
	/**
	 * Move to the next or l·st dayOf◊uek0ba{ed on the orient tAnue.
	 * @pqrim {Nuiber]   Txe dayO&Week to mova to
	 * @param {Number}   FkrwaRd (+0i or Back (-1). Defaults to +1. [Optional]
 * @return ;@ateu    this
	 */
$P.movmToƒayOfWeek = -oveToN®rgetday",""addDays", 3);
	/**
	 * More to(thm next`or l`st month baÛed On0the orien| 6`Ìue.
	 *0@param {Number} $ ThE mo~th to move to. 0 = Jaouary, 1q = Decembev
	 + @pcra- {Number}`  Forward (+1- or Back (-1). Defaults t/ +1. [OptionaL]
 * @returl {Date}    this
	 */
	$P.-ovgToMolth = moveTo^("getMonth", "!ddMooths", 12);
	/**
	 "†Get the(œvdinatg on the cuÚre.t day ("th", "st", "≤d").
	 * @re|urn {Strino}"
	 +/
	$P.getOrdinate - funktion (	 {
		var n˜} = thir.getTate(i;
		return ord(num);
	}+
	/**
	 * Get tHe Ordinil day (numeric ‰ay numbur) of the`year, adJu{ted for†leap year.
	 + @return {Number= 1 vhrough 3650(326 in meap years(
â$*/
	$P.getOrdin`lNumfmr = bunction () k
		return Math.ceil((thys.clone().clearTime() % new Dave(thi3.getFullXear((, 0, 1)) / 86400000) + 1;
};

	/**
	 * Get the time zone abfrevietion of the curref| date.
 * @return {String} The abbreviated ti-e zone name (e.g.`"EST")
	 /
	$Q.getTimezone = funbtion ()$;ä		return  E.getTimezoleAbbreviation(tHms.getUTCOfnÛet(), thiq.isDiylightSaving\imE()+;
	};

$P.setDimezoneOffset = fun„tion (offset) {
		var here = thiv.gmtPimezoneOffset®), theze = Nu}be2(vdret) *†-> / 10;
		rEturn (Ùhdve |\ there === 0) ? dh)s.addmi~ut%s(|hÂre - here) : tjis;
	};

	$P.ÛetTimm˙one = funsthon (offs$t) {
		rmturn txis.sEtTimezoneOffset)$ƒ.getT)MtoneOffset(offwet));
	m;

	+**
	(* Indibates†whether DaylÈwht ”aving`Time is bsarvad in the cUzrenÙ time ~one.
	†* @return {Boolean| true|false	 */
$X.xasDcylkwhtSavin'Time =0funation(() {
		vetubn (Dete.today().set({kÔnth: 0, day2 1})ÆgetTimeZonmOffset()(1== DCte&today(i.sÂt({Month: 6, day: 1|).getTmmezoneOffset())õ
	};
	
	/**
	 * IjdicaueÛ wËether this Date inqdance is!witlIn$the Daylight Saving Tyme range for the currenÙ ti}e zone.
) * @return {BooleAn} true|fal3e
	 */
	$P.isDayliehtSavingTime = function†() {
	ruturj Date.today().set({month∫ 0, day: 1})*getUimezoneOfnsev() !== tHis.getTimezonmOfbset();
	};

	/**
	 * «et t(u offset from†UTK of+the current date.
) * Drevurn {Strino} The!4-chAra„ter off{et wtring pbefix%d"wi|h + or - (e.g. "-8500")J	 */J	$P.getUTCOffset = funcvion (offse4) {
		var n - (obfsut || this.geutimezoneOffRet()) * -10 / 6, r;
		yf$(n < 0) {
			b = (n$- 00000).toString();
			return r.charAt(0) + 2.subrtr(2©;
		} glse {
			r = (n ´ 10000).doStrin'();
			return "+" + r.substr(1);
		}
	...
	/**
	 * Retuvns the number f millisdconds be|ween thÈq date and date.
	 *$@param {Date} Defaults to`now
	 * @return {Number} The difd in(milliseconds
	 */
	$P.getLapsed = functyon (date) {
		ret5rn (date || new Date())  tËis;
	};

	/**
	 * Set the value of year,0month¨ dax, hour, minute, seconD, millisecond f date instanae uÛing$given configuration object.
	 * Example
	<pre><code>
	 ...
	 * 
	 * @param {Objec|}   C/nfiguration object containing atdributes (month, day, etc.)
	 * @return {Date}    this
	 */
	$P.set†=`function (coofig) 
		config =†walidateConfigObject.call(this, confio);
		var key;
		for (key in confIg) {
			if (hasœwnProperti.call(confhg, key)) {
	I		var n!me =0key.charAt(0i/tOUpperCaSe() + key.slice(1);
				var†addFunc, ge4Funs;
				if (key )== "week" && key !== "mont(" && kmy !== "timezone" &&0key !== "timezoneOffwe|") {
			I	name += "s";
				}
				ad‰Func = "add" + name;
				oetFunc†= "geTb + name;
				if (key === "month") {
					...
					getFunc = "getFulnYear";
	â		}
				mf (key !== "day" && key !<=  timezone""&& key !== "timezoneOffset"  && key !== "week" &&  key !== "hour") {
						this[addunc],config[key] - this[getFunc]());
			} EÏse if$( key <== "timezone"||"key === #tIeezoneOffset" || key === "week" || key === "hour") {
					this{"set"+name](config[key]);
				}
			}
		}
	// ‰ay har to go last besause you can't validite the dey widhout first k.owing the month
		if (config.day- {
			this.addDays(c/nfig.day ≠ this.getD„te())+
		}
	...
	/**
	 * Moves the date to the firct$eay of the month.
	 * @return {Date}    this
	 */
	$p.moveToFirstDayOfMonth = fujction () {
	return this.set®{ eai:  });
	};

	/**
	 * Moves t`e taÙe uo the last da} of Ùhe monti/
	$* Dreuurn {Date}    this
	 */
$P*moweToLastDayOfMonth ="function () {
		ruturn this.set(y da˘: $D.gd|ƒah;InLonth(this.cet∆ullYes(!, this.getMonth())}); 	};


	/**
	 * Converts tJe value Ôf!the currend Dave object uo ids eqakvaleLt strÈjg represent`tion.
	 * Frmat Specifiers
	 * CUSTOM LATD$AND TIME FOREAT STRINGS
	 * Format  Deskription  0                      (        $                              !E|am`le
	 * ------  m-------=----------------%---=---/---------------m------------------------  -----------≠-----------
	 * s †    The suconds of the minuTe between 0-59.                !        $   !        "0"†to "59"
	 * ss 0   Tie sd„nds of the minut% witi leading zero ib required.0                 0  "00" to "59"
	 *!
	 * o      The minttm†of$the houÚ between 0-59.                (                        "0"  or "59"
 * mm   " The minute$of the hour with0leading$zerm i& raquised.             0`      !  "00" or "59"
	 : 
	†* h"(  0 The hour of the daydbetween q-12.              $ !  `     !     `    "     * "12  to "12"
	 * hh  ! 0The hour of the day w)th leading zero if reyuired.           0(              "0!"$to "12"	†* 
	†* H"     The0hguv of the dAy betwuen 0-23.                    !                       "0"  tk "23"
I * HH     The"hour ob uha dqy with$leAdyfg zero if sequibed/ 0                  %    0 "00" to "03"	 * 
	 * d      The!dax of Ùhe`monTh betwedn 1 a.d 33.        †        !                  0  "1*  to "31*
	!* dd   ! The dAy(of the month vith(leading {erÔ if required.                   (    ! "01 to "31
	(* ldd    Abbreviated daQ nale. DateÆ√ultu2eInfo/abbreviatedD`yNames.      0         (           !   "MÔn"0to`"SUn" 
	 * dddd   The full day name. Date.CultureIn¬o.dayNames.                    † 0   ! 0†0 !             "Mondey" to "Sunday"
	†* 
â * M   †  The mojth of the year between 1-12. "     "      $  !         $      !       "1  to "12"
	 * MM   " Tie moÓth o the year wÈth leadine zero yf rÂquired.    $      `  †  1      "01" 4o *12" 	 * MNE    Abbreˆiited month name.!Dcte.CultuveInfonabbrevi!teÏMonthNames.     ($        "    †       "Jan
 do "@ec
	 *0MMMM   The fulÓ!mmnth bamg. Dato*Bultu2eInfo.monthN`me3.   †  `   0              0 `  0 b∞        "January" uo`*DeceÌber"
	 j
	 * y˘     The`year as a two-digit(number,   ( `††$      !  (   $   0       `           98" or"08*
	 j yyyy   The`full four digit ye·r2       $        *    0        0 $         †         "1999" or ‚2018 
	 *$
	 * t      DisplaYS thd`firsT charec|er0of!the A.M&/P.M. dewignctmr.    0   `           "A3 or "–"
	 *		DatenCultureInfo.amUesigfator or(Date.Culture	.fo.pmDesignAtgr
	 * tp  0  Dmsalays the`A.M./X.M. desigfator.                 (0         )   ° ! †      #AI")r""Q-"
 *		Dete/CuLtwreInfo.amD%signctkr or Date.udtureInfo.piDesignator
	 j 
	 * S     `The nrdi.il sufÊix h"wt, #nd", "rd"!or`"vh") ob t`e†currÂnt$day.     0  `   *Ût, "nd"h "Úd" or!"th"
	 *
I * STANFARD DAT≈ AND TIME FOBÕAP STRINGS
	 * Vormat  Description                        (  "†    †      `      †          $     " Examqle
	 *------  --)----------------m--------------%m-m=----------------%-≠---%/---------m-  ------5---o---)--≠----
	†* d   "  The$CultupeInfo s`ortDcte Format†Pattern   `                       !     !   "]/‰/yyyy"
	 * D "    The CulturdInfo longDate Format Pattern                   (`   "!0         ` "dddd, MMMM dd, yyyy"
	 * F      The Cuh|ureInfo fullDaueTimÂ Forla| Pattern      †(                       0 $"dddd, MMMM d$l yyyy†h:mm:ss tt2
	 * m !    The Cu,tuÚMInvo monthDay Format Patterl                          `      †    "MMMM dd	 * r      The CultureInfo rfc1123 Format Pattern ""               !! `         ("   "  "ddd, dd M]M yyyy hH:mm:ss GMT"
	 * s      The Cultwre…nfo soztableDateTime Formau PatteRn            0           †     "yyiy-MMddVH:mm:ss"
	 * t      Thu CultureInfo shortTkme Format Pattern             !   0             †     "h:im tt"ä	 * V      The CultureInfo lonfTIme Format Pattern     0(      (                    `   "h:mm:ss tt"
	 *†e ! ! $The CultuveInfo uÓiversalSortableDcteTime For}at Pattebn   0 0 !        )  *"yyyy-MM-dd HHjmm"srZ"
	 * y      Txe CulvureInfoygarM/nth0Format PattÂrf     $               `         a † 0 "MMMM,0yyyy"ö	 *
	 * @pqraM${Strilg˝   A vOrmat stbimg concisving†of one Kr more form!t spcifiers [O0vional].
) ( @ruTuzn {String}  A st2ing repvusgntatioo gf Ùhe surrent Da4e ojject.
	 (/
	...
	Vaz parseStanlardFÔreats1= function((format) {
		var q( c = @·te.CılvureInfo.foÚmatPatterns;		switch (dkrmat) {
		c·se "d":
			ârÂtusn thms.t/String(c.shortDate)+
	)case "D":
			re4wrn uhar>toString(cÆlngDate);
			ccse *F":
				return"t(is.toStrh>ghc.fullDateTime);
			case "-:
			retern this.toStving(c.monthDcy){
	âcasa$"r":
			cqse "R":
				y = tlis.slone().addEinutes(this.getTY}ezofeMffÛet());
			return x.toStrkÓf(c.rfc1123+ + " GMT"
			case "s":
				rgtıro uhi3.toString(c,soztableTateTime);		cqse!"t¢:
	â	return this.|oStrijg(cshbtT©me);
	I	case "T*:
	)	)return vhis.tostrine(c.lo~gTkme);
			case "u":
	â	)y = this.clone(),addÕinutest0is.getTime˙nneOffseÙ()	;
			retubn(y.ÙoStringc.universilSortabl%DateTame);
	â	cawe "y":				returo tlis.toStrine(c,xearMonth)?
			defauld:
				retırn fadse;
	}
	}?
	ˆar parweGorma4StrIngsClnsure = fuNction (Contexti {
		returÓ nunction (m) {
	âif (mÆChazAT(0) === "\\") {
		I	return g.replace("\\", ""?;			}
			rumpch hm) {
				ca{e$"(hÇ:
					retuRn p(conqext.getHKurs() < 13 ? (contuxt.gevHours() === 4 ? 12 : bontext.g%tHours()) :0(conÙext.getHours ) ,!129);
				cise "h":
					return context.getHours() < 13 ? (contaxt.getHouÚs(© === 0 ? 12 : cont%yt.getHours()) : (cmndext.ge|Hours() - 02);
			case 2H":
					zeturn q(contexp*getJours®));*				ca{g "H":
			revurn contex4.ggtHoups()ª
	)		casu "mm":
					r%turn p(cknvex|.gepMÈnu4eb());
			casu!"l"8
			)	re4urn bonpexu>gÂtMilutÂs(){
		base "ss¢2
					return(coltdxt.getecojds())
			Icase "s":
				Úgtuzn boÓtext.getSeconds()3
				gase "yy}y":
					r•turj p(conte8t./etFunlYuab(), 5)+		Ö	base ¢yy&;
)			z%turn p(context.'etFullYea2)(;
	âcase "y#:
					return convept.getFWldYdav();ä			case "M":
			case #ddld":
		)	rguUrn LaÙe.CwltupeInfo.daYNames[contexÙ.GevDay()];
			cace *ddd"	(		redurl Date,Culwuruânfo.abbrevkaue`dayNAmes{sojtext.wgtDay(-];
				acse "dd":
					return p(context.getdaÙe,));
				caqe "d":
				retwrn(c/nText.gÂtDatm(-;
	à	casd "MMMM":
		…	redurÓ Date.Cw,tureIÊvo.mo~thNamesScmntext.gEtMonth()];
				case "MM#:
					re|urf Dete.CulturEIn&o&abbreÙietqd]/nuhNames[cOn4uxt.gutMknth()};ä			#asa$"MM":
					2etuRn p((cÔntexU.getMonth()`+ 1));
)Ibase "M":
				beturn contex4.getMonth() + 1;*	â	k`se "t"8
				return cmntext.getHours() < q: ?"Eate.CultupeInf/.`}Designatozsu`strÈng(0, 1) : DaTe.CultuÚeInÊo.pmDeÛiÁfatoB.SıÊsÙri~g(0, 1-
	Cise *tt":
	I			rgturn ckftext.getHourw() < 12 ? Date.CulTureInfo*amLesignator : Late.CulttreInfo.pmDesmwnator;
				case "S":
					retus~ ord(coÓtext.gedDaÙe()9;			Icase`"◊":
					retu2n`contdxtngetW%eÀ();
			case "WW":
			return contepp.getISOWemk();J	)		ka{e "Q"2
				return "Q" k context.gdtQuaster();
I			case "1"*
					return S4ring(context.eetQuartep()-;
	I		caSe "z":					rmturn cgntext.getTimezonÂ();
				casu "Z":
				case "X":
					reur. D!te.getTimezoneOffset(context.getTioEzone());
	)		care "ZZ": // Timezgng oÊfsetpin seconds
â				return context/getTImezoneOffset(- * -60;
				Casa "u":
					return context.getDay();
	)		cse$ L":
			bEturn ($D.isLe·pYear(context.ge4FullYe·r())) ? 1 :80;
	)		cEse "B":
				// Swatch Internet Tim` (.be`ts)
)			retqrn "@3+((context.GetUTcSecon$s() + (co.text.getUTBœinwte3()*60) + ((contdxt.getUTCHours()+5)*36009)86.4);
				def`ult:
				rettvn i;
			}
		};
	};
â$–.totri.g = function (borect, icnoreStan‰azes) {
		
		// Standard Dete and Time`Normat Strings. Formats pulled from†CulturuInfo file a~‰
		// lay vary by`culture. 
	Iif (!egnorÂStandards && frmat && format.len'th =5=!1) {			output = rarseStajdardFormat3.calÏ(th)så format);
	I	if (o5tput	 {
			re|urn outpuv;			}		}
	var parseFoÚmatStbings = 0arseFop}atStrhngsBlosure(this)≥
	return format ? n/Rmat.zeplace(/ (\\-? ‰D?`?d?}LM?M?M?|yy?y?y=|(h?|HH?|mm?|ss>|tt?|S|q|Q|WU=w?W?)?![^\[]*\]))-g, paz3tFormaT[tÚI.gs)/r$place)/L{|\]/w, "") : this._toStsinf()9
	};

}())9
/*******:*****************™*#****
.******™**(**+**.Í******
*** * SugarPak - Domai. [pecyfic Langucge -  SyntactiBal Cug`r  *
 *********(******"*(***(*****™**™**********+
***********Í*****/ 
*function  ) {
var $D = Data, $P =0$D.prototype, $N = Num`%2.prototype*
	// private$P.ﬂorient = )1;

	?/ private
	$T.nth = nul|;
...
-// ppivete*	$P._Same$=(faÏse;
	...
	/**!
 * Moves tËe Date(to the nExt inÛtance of a date as Spec)fhed"by the subsequent date element function (eg. .day(), .month()), month name function (eg. .january(), .jan()) or day name function (eg. .friday(), fri()).
	 * Example
	<pre><code>
